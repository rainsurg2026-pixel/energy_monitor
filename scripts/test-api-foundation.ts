...
await withApi(false, async (base, authentication) => {
  ...
  const rackUnitSave = await client.request("/api/v1/sites/1/rack-unit-capacity/2026-01", { method: "PUT", body: JSON.stringify({ total_u: 400, available_u: 60, expected_row_version: 1 }) });
  check("Rack Unit Capacity Total + Available save derives Used and persists with optimistic row version", rackUnitSave.status === 200 && rackUnitSave.body.data.rowVersion === 2 && rackUnitSave.body.data.availableU === 60 && rackUnitSave.body.data.usedU === 340);
  ...
  const rackSave = await client.request("/api/v1/racks?siteId=1&month=2026-01", { method: "PUT", body: JSON.stringify({ expected_row_version: 1, changes: [{ row_number: 1, rack_id: "A-01", status: { expected: "In Use", next: "Available" } }] }) });
  check("Rack Capacity batch save returns the confirmed snapshot", rackSave.status === 200 && rackSave.body.data.changedCount === 1 && rackSave.body.data.snapshot.rowVersion === 2 && rackSave.body.data.snapshot.records[0].status === "Available");
  const rackConflict = await client.request("/api/v1/racks?siteId=1&month=2026-01", { method: "PUT", body: JSON.stringify({ expected_row_version: 2, changes: [{ row_number: 1, rack_id: "A-01", status: { expected: "In Use", next: "Reserved" } }] }) });
  check("Rack Capacity field conflict is a distinguishable atomic 409", rackConflict.status === 409 && rackConflict.body.error?.code === "RACK_CAPACITY_CONFLICT");
  const missingRackSave = await client.request("/api/v1/racks?siteId=2&month=2026-02", { method: "PUT", body: JSON.stringify({ expected_row_version: null, changes: [{ row_number: 1, rack_id: "A-01", status: { expected: "Available", next: "Reserved" } }] }) });
  check("Rack Capacity save maps a missing monthly snapshot consistently", missingRackSave.status === 404 && missingRackSave.body.error?.code === "RACK_CAPACITY_NOT_FOUND");
