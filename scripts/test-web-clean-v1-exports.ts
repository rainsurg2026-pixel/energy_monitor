...
const groupedNames = ["02_Saved_Energy", "03_Saved_Rack", "04_Saved_RackUnit", "05_Input_UPS", "06_Input_AirConditioning", "07_Input_DCPower", "08_Input_Rack", "09_History_Energy", "10_History_Rack"];
check("Current Facility workbook uses Saved -> Input -> History sheet order", groupedNames.every((name, index) => completeSheetNames.indexOf(name) > completeSheetNames.indexOf("01_Dashboard") && (index === 0 || completeSheetNames.indexOf(name) > completeSheetNames.indexOf(groupedNames[index - 1]))));
...
check("Current Facility Excel first sheet shows the export timestamp", selectionDashboard.getCell("K3").value === auditTimestamp);
...
check("All Facilities Excel first sheet shows the authenticated display name", multiAuditFirstSheet.getCell("B4").value === auditUser);
check("All Facilities Excel first sheet shows the export timestamp", multiAuditFirstSheet.getCell("H4").value === auditTimestamp);
...
check("Current Facility PDF cover shows the authenticated display name", auditReportHtml.includes(`Generated by: ${auditUser}`));
check("Current Facility PDF cover shows the export timestamp", auditReportHtml.includes(`Generated at: ${auditTimestamp}`));
...
check("All Facilities PDF cover shows the authenticated display name", allAuditHtml.includes(`Generated by: ${auditUser}`));
check("All Facilities PDF cover shows the export timestamp", allAuditHtml.includes(`Generated at: ${auditTimestamp}`));
...

{
  ...
  const currentGroups = ["02_Saved_Energy", "03_Saved_Rack", "04_Saved_RackUnit", "05_Input_UPS", "06_Input_AirConditioning", "07_Input_DCPower", "08_Input_Rack", "09_History_Energy", "10_History_Rack"];
