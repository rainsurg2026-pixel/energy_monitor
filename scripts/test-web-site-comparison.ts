...
assert.match(comparison, /function withLeadingChartGap/);
assert.match(comparison, /function comparisonChartYAxisDomain/);
assert.match(comparison, /const chartData = withLeadingChartGap/);
assert.match(comparison, /<LineChart data=\{chartData\} margin=\{\{ top: 24, right: 28, left: 12, bottom: 24 \}\}>/);
assert.match(comparison, /<YAxis domain=\{yDomain\}/);
