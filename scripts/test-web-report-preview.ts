...
assert.match(app, /import \{ buildAllFacilitiesReportHtml, buildSiteComparisonReportHtml,/);
assert.match(app, /overrideHtml=\{exportScope === "current" \? null : scopedPreview\?\.html \?\? null\}/);
assert.match(app, /html = buildAllFacilitiesReportHtml\(facilities, contextMonth, selectedReportSections\)/);
