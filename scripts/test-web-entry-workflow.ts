...
const toolbar = readFileSync(new URL("../src/components/StickyEntryToolbar.tsx", import.meta.url), "utf8");
const completion = readFileSync(new URL("../src/utils/completion.ts", import.meta.url), "utf8");
const rackUnitEntry = readFileSync(new URL("../src/web-clean-v1/RackUnitCapacityEntry.tsx", import.meta.url), "utf8");
...
assert.match(workspace, /rackUnitCompletion/);
assert.match(workspace, /entry-section-rack-unit/);
assert.match(workspace, /onCompletionChange/);
...
assert.ok(acCalc.airEnergyKwh!==null&&Math.abs(acCalc.airEnergyKwh-200000)<0.001);
...
assert.match(app, /if \(!unsavedChangesRef\.current\)/);
...
assert.match(workspace, /onDirtyChange=\{setRackUnitDirty\}/);
assert.match(workspace, /rackUnitActionsRef/);
assert.match(workspace, /rackUnitActions\.hasChanges\(\)/);
...
assert.match(desktopHeader, /showHealth\?: boolean/);
assert.match(completion, /rackUnit: SectionCompletion/);
assert.match(toolbar, /sectionSummary\("rackUnit"/);
...
assert.match(rackUnitEntry, /RackUnitCapacityEntryActions/);
assert.match(rackUnitEntry, /sameNumericValue/);
assert.match(rackUnitEntry, /onDirtyChange/);
assert.match(rackUnitEntry, /if \(busy\) return false/);
assert.match(rackUnitEntry, /disabled=\{busy\}/);
