...
export class PostgresAuthRepository implements AuthRepository {
  ...
  async deleteUser(targetUserId: string, actorUserId: string, correlationId: string): Promise<void> {
    await this.inTransaction(async repository => {
      ...
      // sessions.user_id is ON DELETE CASCADE, so the DELETE below removes
      // every session row regardless - this explicit revoke+audit exists so
      // the audit trail records the revocation the same way it does for
      // deactivate/password-reset, matching that pattern rather than
      // relying on a schema-level cascade to be the only record of intent.
      await repository.revokeAllSessions(targetUserId, "user_deleted");
      await repository.audit({ actorUserId, action: "SESSION_REVOKED_ALL", entityType: "user", entityId: targetUserId, newValue: { reason: "user_deleted" }, correlationId });
      await repository.audit({ actorUserId, action: "user_delete", entityType: "user", entityId: targetUserId, previousValue: { username: before.username, display_name: before.displayName, role: before.role, active: before.active }, correlationId });
...
export class InMemoryAuthRepository implements AuthRepository {
  ...
  async deleteUser(targetUserId: string, actorUserId: string, correlationId: string): Promise<void> { const user = this.users.get(targetUserId); if (!user) throw new HttpError(404, "USER_NOT_FOUND", "User was not found."); if (user.active && user.role === "admin" && [...this.users.values()].filter(item => item.active && item.role === "admin" && item.id !== targetUserId).length === 0) throw new HttpError(409, "LAST_ADMIN", "At least one active admin must remain."); await this.revokeAllSessions(targetUserId, "user_deleted"); await this.audit({ actorUserId, action: "SESSION_REVOKED_ALL", entityType: "user", entityId: targetUserId, newValue: { reason: "user_deleted" }, correlationId }); await this.audit({ actorUserId, action: "user_delete", entityType: "user", entityId: targetUserId, previousValue: { username: user.username, display_name: user.displayName, role: user.role, active: user.active }, correlationId }); this.users.delete(targetUserId); for (const [sessionId, session] of this.sessions) if (session.userId === targetUserId) this.sessions.delete(sessionId); }
