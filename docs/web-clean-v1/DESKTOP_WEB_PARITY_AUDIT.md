...
| Rack Capacity/Utilization | dedicated Desktop nav section; XLSM Rack Capacity/Rack Unit Capacity/Rack Capacity History sheets | Rack Capacity History and Rack Unit Capacity are persisted/read through the API; final Data Entry includes numeric Rack Unit Capacity plus PNG/JPEG attachment; XLSX export includes Rack Capacity History, Rack Unit Capacity and attachment metadata | STATIC/API VERIFIED | Authenticated live browser UAT and remote data verification remain unverified; binary images remain in server-side storage and are not embedded in XLSX |
