...
5. **Database**: migrations `003`–`011` must be applied to the **Production** Supabase project (`tofdgndrrpnnyhbuurbx`) before or immediately as part of deploy — they are additive-only (verified: zero `DROP TABLE`/`DROP COLUMN`/`DROP CONSTRAINT` across all of them), safe to apply ahead of the code that depends on them. Apply in filename order, exactly as done on Preview. **Do not apply migration `011` if Production has independently decided against ever restoring Google Backup** — it only adds a nullable, harmless column (`backup_config.connected_google_user_id`) that current code no longer reads either way, so applying it is safe but not required.
