...
| Vercel Preview runtime (Phase 7.1) | **Deployed; unauthenticated checks pass; full UAT Pending External Credentials** | Live at `https://energy-monitor-git-feat-web-v3-dcm15.vercel.app` (Vercel build+deploy succeeded). Confirmed without credentials: `/api/v1/health` 200, `/api/v1/readiness` 200 with `{"status":"ready"}` (the Supabase pooler-connectivity fix works against the real deployment), `/api/v1/bootstrap` correctly 401s unauthenticated, CSRF cookie has correct `Secure; SameSite=Lax` attributes, SPA routing serves `/login` correctly. The authenticated portion of `scripts/test-preview-http.ts` (login/RBAC/migrated-data/read-only-mode checks) has not been run — it needs `DEV_ADMIN_PASSWORD`/`PREVIEW_UAT_PASSWORD`, which are not available in this environment. See `docs/web-v3/PREVIEW_VERIFICATION.md` to run it. | High — Pending External Credentials, not a code or environment defect |
