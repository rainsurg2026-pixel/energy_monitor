...
| Auth architecture reuse from `mqr-webapp-new` | Done (divergent-by-design) | Documented in `docs/authentication.md` § "Architecture decision: divergence from the mqr-webapp-new reference" — different framework/session/RBAC mechanics, equivalent security properties, independently verified | — |
