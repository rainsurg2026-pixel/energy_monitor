...
| Vercel Preview runtime (Phase 7.1) | **Deployed; unauthenticated + auth-reachability checks pass; full UAT Pending External Credentials** | Live at `https://energy-monitor-git-feat-web-v3-dcm15.vercel.app`. Confirmed without credentials: `/api/v1/health` 200, `/api/v1/readiness` 200 with `{"status":"ready"}`, `/api/v1/bootstrap` correctly 401s unauthenticated, CSRF cookie has correct `Secure; SameSite=Lax` attributes, SPA routing serves `/login` correctly. **Root-caused and fixed an `ORIGIN_NOT_ALLOWED` rejection** that blocked auth entirely: CORS's exact-match origin allowlist had no way to know Vercel's per-deployment hostname without hand-maintaining `APP_PREVIEW_ORIGINS` per branch/deployment. Fixed by trusting each deployment's own `VERCEL_URL`/`VERCEL_BRANCH_URL` automatically (`server/config/env.ts`) — still exact-match, no wildcard, verified live: a request with a deliberately wrong password from this deployment's own origin now returns `INVALID_CREDENTIALS` (proving auth logic is reached), while a request from an unrelated origin still gets `403 ORIGIN_NOT_ALLOWED`. With the correct password this would succeed — that's the one thing left needing a human. The rest of `scripts/test-preview-http.ts` (RBAC/migrated-data/read-only-mode checks) needs `DEV_ADMIN_PASSWORD`/`PREVIEW_UAT_PASSWORD`, not available in this environment. See `docs/web-v3/PREVIEW_VERIFICATION.md` to run it. | High — Pending External Credentials, not a code or environment defect |
...
| Production deployment | **Not started — Production Ready, Awaiting Manual UAT** | Everything achievable without credentials is done: pushed, deployed, code-reviewed, tested (see `PRODUCTION_VERIFICATION_CHECKLIST.md`), PR opened. Remaining: (1) a human with the Preview account passwords runs `scripts/test-preview-http.ts` per `docs/web-v3/PREVIEW_VERIFICATION.md` and reports the result, (2) explicit Product Owner approval to merge `feat/web-v3` into `main` per `.claude/rules/git.md` | Blocking — see `docs/web-v3/PHASE7_1_VERCEL_PREVIEW.md`'s own "does not authorize production deployment" line |
