...
import { Check, ChevronDown, ChevronUp, FileDown, FileText, Monitor, RotateCcw, X } from "lucide-react";
...
export type ReportingFormat = "pdf" | "excel" | "html" | "powerpoint";
...
interface Props {
  ...
  onPreview: (month: string) => Promise<void>;
...
export default function ReportingCenter(props: Props) {
  const initialMonth = props.availableMonths.at(-1) ?? new Date().toISOString().slice(0, 7);
...
function Workspace({ facility, availableMonths, previewHtml, previewStatus, previewError, onPreview, onGenerate, onReveal }: Props) {
  ...
  useEffect(() => { void onPreview(month); }, [month, onPreview]);
