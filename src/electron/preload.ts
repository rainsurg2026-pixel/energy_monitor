...
const desktopBridge = {
  ...
  excel: {
    ...
    saveRackCapacity: (payload: { path: string; changes: unknown[]; image?: { bytes: Uint8Array } | null; facilityId?: string | null; snapshotMonth?: string | null }) => invoke("excel:saveRackCapacity", payload),
    saveRackUnitCapacity: (payload: { path: string; input: { month: string; totalU: number; usedU: number }; image?: { bytes: Uint8Array } | null }) => invoke("excel:saveRackUnitCapacity", payload),
