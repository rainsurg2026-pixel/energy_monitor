...
function currentExecutiveDashboardPage(data: ReportData): string {
  ...
  return `<section class="page executive-dashboard-page executive-summary-flow" data-report-section="executive"><div class="dashboard-head"><div><p class="eyebrow">EXECUTIVE VIEW</p><h2>Executive View</h2><p>${escapeHtml(data.facility)} · ${escapeHtml(formatMonth(current.month))} · ${monthDays ?? "—"} Days · ${escapeHtml(source)}</p></div><div class="dashboard-tag">Generated<br>${escapeHtml(formatBangkokReportTimestamp(data.generatedAt))}</div></div><div class="kpis-3col">${kpi("Building Energy", format2(current.buildingEnergyKwh), "kWh", pctDelta(current.buildingEnergyKwh, previous?.buildingEnergyKwh ?? null))}${kpi("Building Cost", format2(current.buildingCostThb), "THB", pctDelta(current.buildingCostThb, previous?.buildingCostThb ?? null))}${kpi("4th Floor Energy", format2(current.floorEnergyKwh), "kWh", pctDelta(current.floorEnergyKwh, previous?.floorEnergyKwh ?? null))}${kpi("Estimated 4th Floor Cost", format2(current.floorCostThb), "THB", pctDelta(current.floorCostThb, previous?.floorCostThb ?? null))}${kpi("4th Floor Energy Share", `${format2(current.floorSharePercent)}%`, "of building energy", pctDelta(current.floorSharePercent, previous?.floorSharePercent ?? null))}${kpi("Average Electricity Rate", format2(current.averageRateThbPerKwh), "THB/kWh", pctDelta(current.averageRateThbPerKwh, previous?.averageRateThbPerKwh ?? null))}</div></section>`;
}
