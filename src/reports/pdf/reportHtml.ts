...
export function reportCoverMain(data: ReportData): string {
  ...
  const audit = `${data.generatedBy ? `<br>Generated by: ${escapeHtml(data.generatedBy)}` : ""}<br>Generated at: ${escapeHtml(data.generatedAt)}`;
  return `<main class="cover"><h1>${escapeHtml(data.title)}</h1><h2>${escapeHtml(data.thaiSubtitle)}</h2><div class="meta">Facility: ${escapeHtml(data.facility)}<br>Reporting month: ${escapeHtml(formatMonth(data.reportingMonth))}<br>Historical range: ${escapeHtml(range)}${audit}</div></main>`;
}
