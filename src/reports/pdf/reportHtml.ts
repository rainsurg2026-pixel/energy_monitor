...
.capacity-overview-page{height:183mm;overflow:hidden;break-inside:avoid}.capacity-overview-page .kpi{min-height:66px}.executive-capacity-trends{display:grid;grid-template-columns:1fr;gap:6px;margin-top:7px}.executive-capacity-trends .block{margin:0;padding:6px 9px}.executive-capacity-trends h3{font-size:15px;margin-bottom:1px}.executive-capacity-trends .note{font-size:9.5px;margin-top:1px}.executive-capacity-trends svg{height:148px!important}.executive-capacity-trends .trend-legend{justify-content:center;margin-top:-2px}.executive-capacity-trends .trend-legend span{font-size:11px}
...
/* Engineering View is one continuous report flow. Its two semantic wrappers may
 * cross physical A4 pages naturally so a late comparison block never occupies
 * an otherwise empty page before the continuation section starts. */
.dashboard-continuation-page{min-height:0;page-break-before:auto;break-before:auto;page-break-inside:auto;break-inside:auto}
.executive-summary-flow{min-height:0;page-break-before:always;break-before:page;page-break-after:auto;break-after:auto;padding-bottom:8px}
...
function facilityTrendSummaryPage(data: ReportData): string {
  const rows = data.executiveTrendRows ?? data.monthlyRows;
  if (rows.length === 0) return "";
  const summarize = (pick: (row: ReportMonthlyRow) => number | null) => {
  ...
  const buildingEnergy = summarize(row => row.buildingEnergyKwh);
  const floorEnergy = summarize(row => row.floorEnergyKwh);
  const buildingCost = summarize(row => row.buildingCostThb);
  const floorCost = summarize(row => row.floorCostThb);
  return `<section class="page facility-trend-summary-page" data-report-section="historical"><p class="eyebrow">FACILITY TREND ANALYTICS</p><h2>Facility Trend Analytics Summary</h2><p class="note">${escapeHtml(data.facility)} · ${rows.length} persisted month${rows.length === 1 ? "" : "s"} ending at ${escapeHtml(formatMonth(rows.at(-1)?.month ?? null))}</p><div class="kpis">${kpi("Building Energy Total", format2(buildingEnergy.total), "kWh", "Selected trend window")}${kpi("4th Floor Energy Total", format2(floorEnergy.total), "kWh", "Selected trend window")}${kpi("Building Energy Monthly Average", format2(buildingEnergy.average), "kWh", "Persisted months only")}${kpi("4th Floor Energy Monthly Average", format2(floorEnergy.average), "kWh", "Persisted months only")}${kpi("Building Cost Total", format2(buildingCost.total), "THB", "Selected trend window")}${kpi("4th Floor Cost Total", format2(floorCost.total), "THB", "Selected trend window")}${kpi("Building Cost Monthly Average", format2(buildingCost.average), "THB", "Persisted months only")}${kpi("4th Floor Cost Monthly Average", format2(floorCost.average), "THB", "Persisted months only")}</div></section>`;
}
...
function executiveDashboardPage(data: ReportData): string {
  ...
  return `<section class="page executive-dashboard-page" data-report-section="executive"><div class="dashboard-head"><div><p class="eyebrow">EXECUTIVE DASHBOARD</p><h2>Executive Dashboard</h2><p>${escapeHtml(data.facility)} · ${escapeHtml(formatMonth(data.reportingMonth))} · ${rows.length} reporting month(s)</p></div><div class="dashboard-tag">Management summary<br>${escapeHtml(formatMonth(data.reportingMonth))}</div></div><div class="kpis-3col">${kpi("Building Energy", format2(buildingEnergy), "kWh", `${rows.length} reporting month(s)`)}${kpi("4th Floor Energy", format2(floorEnergy), "kWh", "UPS + AC + DC power panels")}${kpi("Building Cost", format2(buildingCost), "THB", "Stored/calculated building cost")}${kpi("Estimated 4th Floor Cost", format2(floorCost), "THB", "Calculated at building average rate")}</div><article class="block"><h3>Management insights</h3><ul class="insight-list">${insights.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ul></article></section>`;
}
...
function currentExecutiveDashboardPage(data: ReportData): string {
  ...
  return `<section class="page executive-dashboard-page executive-summary-flow" data-report-section="executive"><div class="dashboard-head"><div><p class="eyebrow">EXECUTIVE VIEW</p><h2>Executive View</h2><p>${escapeHtml(data.facility)} · ${escapeHtml(formatMonth(current.month))} · ${monthDays ?? "—"} Days · ${escapeHtml(source)}</p></div><div class="dashboard-tag">Generated<br>${escapeHtml(formatBangkokReportTimestamp(data.generatedAt))}</div></div><div class="kpis-3col">${kpi("Building Energy", format2(current.buildingEnergyKwh), "kWh", pctDelta(current.buildingEnergyKwh, previous?.buildingEnergyKwh ?? null))}${kpi("Building Cost", format2(current.buildingCostThb), "THB", pctDelta(current.buildingCostThb, previous?.buildingCostThb ?? null))}${kpi("4th Floor Energy", format2(current.floorEnergyKwh), "kWh", pctDelta(current.floorEnergyKwh, previous?.floorEnergyKwh ?? null))}${kpi("Estimated 4th Floor Cost", format2(current.floorCostThb), "THB", pctDelta(current.floorCostThb, previous?.floorCostThb ?? null))}</div></section>`;
}
...
function compactExecutiveTrendChart(title: string, labels: string[], series: TrendSeries[], unit: string): string {
  const width = 960, height = 205, left = 62, right = 20, top = 30, bottom = 42;
  ...
  const ticks = [0, 1, 2, 3].map(step => {
    ...
    const yy = y(value);
    return `<line x1="${left}" y1="${yy}" x2="${width - right}" y2="${yy}" stroke="#e2e8f0"/><text x="${left - 5}" y="${yy + 3}" text-anchor="end" font-size="10" fill="#64748b">${escapeHtml(compactNumber(value, values))}</text>`;
  }).join("");
  const paths = series.map((item, seriesIndex) => {
    const points = item.values.map((value, index) => {
      ...
      const nearby = series
        ...
        .filter(candidate => Math.abs(candidate.y - pointY) < 20)
        .sort((left, right) => left.index - right.index);
      const rank = Math.max(0, nearby.findIndex(candidate => candidate.index === seriesIndex));
      const tier = Math.floor(rank / 2);
      const offset = nearby.length > 1 ? (rank % 2 === 0 ? -(11 + tier * 11) : 15 + tier * 11) : -8;
      const labelY = Math.max(top + 8, Math.min(height - bottom - 5, pointY + offset));
      return `<circle cx="${x(index)}" cy="${pointY}" r="3.2" fill="#fff" stroke="${item.color}" stroke-width="1.5"/><text x="${x(index)}" y="${labelY}" text-anchor="middle" font-size="9" font-weight="700" fill="${item.color}">${escapeHtml(compactNumber(value, values))}</text>`;
    }).join("");
    return `<path d="${pathFor(item)}" fill="none" stroke="${item.color}" stroke-width="2.6" stroke-linecap="round"/>${points}`;
  }).join("");
  const labelIndexes = labels.length <= 3 ? labels.map((_, index) => index) : [0, Math.floor((labels.length - 1) / 2), labels.length - 1];
  const xLabels = labelIndexes.map(index => `<text x="${x(index)}" y="${height - 9}" text-anchor="middle" font-size="10" font-weight="600" fill="#64748b">${escapeHtml(labels[index] ?? "")}</text>`).join("");
  const legend = series.map(item => `<span><i style="background:${item.color}"></i>${escapeHtml(item.name)}</span>`).join("");
  return `<article class="block" style="min-width:0"><h3 style="color:${series[0]?.color ?? "#29415d"}">${escapeHtml(title)}</h3><p class="note">Last ${labels.length} persisted month${labels.length === 1 ? "" : "s"} · ${escapeHtml(unit)}</p><svg viewBox="0 0 ${width} ${height}" style="width:100%;height:176px">${ticks}<line x1="${left}" y1="${height - bottom}" x2="${width - right}" y2="${height - bottom}" stroke="#94a3b8"/>${paths}${xLabels}</svg><div class="trend-legend">${legend}</div></article>`;
}
...
function currentExecutiveCapacityPage(data: ReportData): string {
  ...
  const hasRackTrend = rackRows.some(row => row.usagePct !== null || row.availabilityPct !== null);
  const hasUnitTrend = unitRows.length > 0;
  if (rackUsage === null && availableRacks === null && unitCurrent === null && !hasRackTrend && !hasUnitTrend) return "";
...
function currentExecutiveTrendPages(data: ReportData): string {
  ...
  return charts.map(([title, unit, color, values, explanation], index) => {
    const page = trendPage(title, unit, [{ name: title, color, values }], rows, explanation, eyebrow, "executive");
    return index === 0 ? page.replace('class="page trend-page"', 'class="page trend-page executive-first-trend-continuation"') : page;
  }).join("");
}
...
/** Current Facility PDF only: Engineering -> Executive -> Rack Capacity -> Rack Unit Capacity. Other formats
...
export function buildReportBodyPages(data: ReportData, selectedSections?: readonly ReportSectionId[]): string {
  ...
  const body =
    ...
    facilityTrendSummaryPage(data) + trendPages +
    appendix +
...
export function reportCoverMain(data: ReportData): string {
  ...
  const generatedBy = data.generatedBy?.trim() || "N/A";
  const generatedAt = formatBangkokReportTimestamp(data.generatedAt);
  return `<main class="cover">
    <div class="cover-kicker">Monthly Energy &amp; Facility Report</div>
    <h1>${escapeHtml(data.title)}</h1>
    <h2>${escapeHtml(data.thaiSubtitle)}</h2>
    <div class="cover-rule"></div>
    <div class="cover-meta-grid">
      <div class="cover-meta-card"><span class="cover-meta-label">Facility</span><span class="cover-meta-value">${escapeHtml(data.facility)}</span></div>
      <div class="cover-meta-card"><span class="cover-meta-label">Reporting month</span><span class="cover-meta-value">${escapeHtml(formatMonth(data.reportingMonth))}</span></div>
      <div class="cover-meta-card"><span class="cover-meta-label">Historical range</span><span class="cover-meta-value">${escapeHtml(range)}</span></div>
    </div>
    <div class="cover-audit">
      <div class="cover-audit-item"><span class="cover-meta-label">Generated by</span><span class="cover-meta-value">${escapeHtml(generatedBy)}</span></div>
      <div class="cover-audit-item"><span class="cover-meta-label">Generated at</span><span class="cover-meta-value">${escapeHtml(generatedAt)}</span></div>
    </div>
