  ...
  return <section className="space-y-3" data-testid={`executive-capacity-${layout}`}>
    ...
    <div className={compact ? "space-y-3" : "grid grid-cols-2 gap-4"}>
      <article className={`min-w-0 rounded-2xl border border-slate-800/70 bg-slate-900 shadow-sm ${compact ? "p-4" : "p-5"}`}>
        ...
        {rackTrend.length ? <div className="mt-3"><TrendLineChart labels={rackTrend.map(row => formatMonthYear(row.snapshotMonth))} unit="%" height={compact ? 230 : 270} series={[{ name: "Usage %", color: "#6366f1", values: rackTrend.map(row => row.usagePct === null ? null : row.usagePct * 100) }, { name: "Availability %", color: "#14b8a6", values: rackTrend.map(row => row.availabilityPct === null ? null : row.availabilityPct * 100) }]} /></div> : <p className="mt-5 text-sm text-slate-500">{copy.noTrend}</p>}
      </article>
      <article className={`min-w-0 rounded-2xl border border-slate-800/70 bg-slate-900 shadow-sm ${compact ? "p-4" : "p-5"}`}>
        ...
        {unitTrend.length ? <div className="mt-3"><TrendLineChart labels={unitTrend.map(row => formatMonthYear(row.month))} unit="U" height={compact ? 230 : 270} series={[{ name: "Total U", color: "#64748b", values: unitTrend.map(row => row.totalU) }, { name: "Used U", color: "#6366f1", values: unitTrend.map(row => row.usedU) }, { name: "Available U", color: "#14b8a6", values: unitTrend.map(row => row.availableU) }]} /></div> : <p className="mt-5 text-sm text-slate-500">{copy.noTrend}</p>}
      </article>
    </div>
  </section>;
}
