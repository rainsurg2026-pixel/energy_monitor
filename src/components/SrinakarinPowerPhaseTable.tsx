...
export default function SrinakarinPowerPhaseTable({
  ...
  const ppc43CurrentRows = Object.entries(inputs.ppc43Current).sort(([a], [b]) => a.localeCompare(b));
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm space-y-0">
      ...
      <section className="p-5 border-b border-slate-800">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left border-collapse"><thead><tr className="bg-indigo-950/30 text-[11px] font-normal uppercase tracking-wider text-indigo-300 border-b border-slate-800/80"><th className="py-2 px-3">Month</th><th className="py-2 px-3">UPS</th><th className="py-2 px-3 text-right">Voltage (V)</th><th className="py-2 px-3 text-right">Current (A)</th><th className="py-2 px-3 text-right">Load (kW)</th><th className="py-2 px-3 text-right">Load (kVA)</th></tr></thead><tbody className="divide-y divide-slate-800/70">{upsRows.map(([id, row]) => <tr key={id} className="hover:bg-slate-850/40 transition-colors"><td className="py-2 px-3 text-xs text-slate-500">{formatMonthYear(monthStr)}</td><td className="py-2 px-3 text-sm text-slate-200">{id}</td>{(["voltage", "current", "loadKw", "loadKva"] as const).map(field => <td key={field} className="py-2 px-2"><input type="number" step={field === "voltage" || field === "current" ? "0.1" : "0.01"} value={row[field] ?? ""} onChange={event => updateUpsPhase(id, field, event.target.value)} className={inputClass()} /></td>)}</tr>)}</tbody></table></div>
      </section>
      ...
      <section className="p-5 border-b border-slate-800">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left border-collapse"><thead><tr className="bg-teal-950/30 text-[11px] font-normal uppercase tracking-wider text-teal-300 border-b border-slate-800/80"><th className="py-2 px-3">Month</th><th className="py-2 px-3">AC Power Panel</th><th className="py-2 px-3 text-right">Voltage (V)</th><th className="py-2 px-3 text-right">Current (A)</th></tr></thead><tbody className="divide-y divide-slate-800/70">{acRows.map(([id, row]) => <tr key={id} className="hover:bg-slate-850/40 transition-colors"><td className="py-2 px-3 text-xs text-slate-500">{formatMonthYear(monthStr)}</td><td className="py-2 px-3 text-sm text-slate-200">{id}</td><td className="py-2 px-2"><input type="number" step="0.1" value={row.voltage ?? ""} onChange={event => updateAcPhase(id, "voltage", event.target.value)} className={inputClass()} /></td><td className="py-2 px-2"><input type="number" step="0.1" value={row.current ?? ""} onChange={event => updateAcPhase(id, "current", event.target.value)} disabled={isPpc43(id)} className={inputClass(isPpc43(id))} /></td></tr>)}</tbody></table></div>
      </section>

      <section className="p-5 border-b border-slate-800">
        <h4 className="text-sm text-slate-100">3. Manual PPC Aggregate Load</h4>
        <p className="text-xs text-slate-500 mt-1 mb-4">PPC44 Load kW/kVA เป็นค่ากรอกมือ ไม่คำนวณแทนจาก PPC43</p>
        <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left border-collapse"><thead><tr className="bg-amber-950/20 text-[11px] font-normal uppercase tracking-wider text-amber-300 border-b border-slate-800/80"><th className="py-2 px-3">Month</th><th className="py-2 px-3">AC Power Panel</th><th className="py-2 px-3 text-right">Load (kW)</th><th className="py-2 px-3 text-right">Load (kVA)</th></tr></thead><tbody className="divide-y divide-slate-800/70">{PPC_MANUAL_LOAD_IDS.map(id => <tr key={id} className="hover:bg-slate-850/40 transition-colors"><td className="py-2 px-3 text-xs text-slate-500">{formatMonthYear(monthStr)}</td><td className="py-2 px-3 text-sm text-slate-200">{id}</td><td className="py-2 px-2"><input type="number" step="0.01" value={manualLoads[id]?.loadKw ?? ""} onChange={event => updateManualLoad(id, "loadKw", event.target.value)} className={inputClass()} /></td><td className="py-2 px-2"><input type="number" step="0.01" value={manualLoads[id]?.loadKva ?? ""} onChange={event => updateManualLoad(id, "loadKva", event.target.value)} className={inputClass()} /></td></tr>)}</tbody></table></div>
      </section>
      ...
      <section className="p-5 border-b border-slate-800">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left border-collapse"><thead><tr className="bg-sky-950/20 text-[11px] font-normal uppercase tracking-wider text-sky-300 border-b border-slate-800/80"><th className="py-2 px-3">Month</th><th className="py-2 px-3">AC Power Panel</th><th className="py-2 px-3 text-right">Panel Current (A)</th></tr></thead><tbody className="divide-y divide-slate-800/70">{ppc43CurrentRows.map(([id, value]) => <tr key={id} className="hover:bg-slate-850/40 transition-colors"><td className="py-2 px-3 text-xs text-slate-500">{formatMonthYear(monthStr)}</td><td className="py-2 px-3 text-sm text-slate-200">{id}</td><td className="py-2 px-2"><input type="number" step="0.1" value={value ?? ""} onChange={event => updatePpc43Current(id, event.target.value)} className={inputClass()} /></td></tr>)}</tbody></table></div>
      </section>
      ...
      <section className="p-5 border-b border-slate-800">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left border-collapse"><thead><tr className="bg-violet-950/20 text-[11px] font-normal uppercase tracking-wider text-violet-300 border-b border-slate-800/80"><th className="py-2 px-3">Month</th><th className="py-2 px-3">AC Power Panel</th><th className="py-2 px-3 text-right">Load (kW)</th><th className="py-2 px-3 text-right">Load (kVA)</th></tr></thead><tbody className="divide-y divide-slate-800/70">{ppc43PanelRows.map(([id, row]) => <tr key={id} className="hover:bg-slate-850/40 transition-colors"><td className="py-2 px-3 text-xs text-slate-500">{formatMonthYear(monthStr)}</td><td className="py-2 px-3 text-sm text-slate-200">{id}</td>{(["loadKw", "loadKva"] as const).map(field => <td key={field} className="py-2 px-2"><input type="number" step="0.01" value={row[field] ?? ""} onChange={event => updatePpc43Panel(id, field, event.target.value)} className={inputClass()} /></td>)}</tr>)}</tbody></table></div>
      </section>
