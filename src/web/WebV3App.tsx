...
function ReadDataPage({ kind, siteId, month }: { kind: "dashboard" | "energy" | "cost" | "electrical" | "racks" | "rack-units"; siteId: number; month: string }) {
  ...
  if (kind === "racks") { const metrics = data.snapshot?.metrics; return <div className="space-y-5"><h1 className="text-3xl font-semibold">Racks — {data.month}</h1>{data.snapshot ? <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3"><MetricCard label="Total Racks" value={metrics?.totalRacks} /><MetricCard label="Occupied" value={metrics?.occupiedRacks} /><MetricCard label="Available" value={metrics?.availableRacks} /><MetricCard label="Utilization" value={metrics?.utilizationPercent} unit="%" /></div> : <EmptyState />}</div>; }
