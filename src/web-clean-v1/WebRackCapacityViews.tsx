...
function RackUnitCapacityDashboardInner({ month, facilityName, imageProvider, history, lang }: { month: string; facilityName: string | null; imageProvider?: Pick<IDataProvider, "getRackUnitCapacityImage"> | null; history: RackCapacityHistoryRow[]; lang: AppLanguage }) {
  ...
  const usageLevel = usage === null ? null : rackUtilizationLevel(usage);
  const usageColors = levelColors(usageLevel);
  const heroProgress = clampPercent(usage);
  const historySorted = [...rackUnitCapacity].sort((a, b) => a.month.localeCompare(b.month));

  return <div data-testid="rack-unit-dashboard-v3" className="space-y-5">
    <div className="rounded-[26px] border border-slate-800/80 bg-[#061427] p-3 shadow-[0_28px_90px_rgba(2,6,23,0.4)] sm:p-5">
      <header className="relative min-h-[245px] overflow-hidden rounded-[22px] border border-blue-500/20 bg-[#081b33] p-5 sm:p-7">
        <RackHeroBackground />
        <div className="relative z-10 flex h-full flex-col justify-between gap-8 xl:flex-row xl:items-center">
          <div className="max-w-3xl">
            <div className="flex items-start gap-3">
              <div className="rounded-xl border border-blue-500/25 bg-blue-500/10 p-2.5 text-blue-300"><Boxes className="h-6 w-6" /></div>
              <div><h2 className="font-display text-2xl font-bold tracking-tight text-slate-50 sm:text-3xl">Rack Unit Capacity &amp; Utilization — {monthLabelLong(month, "en")}</h2><p className="mt-1 text-sm text-slate-400">Physical rack-space capacity, usage and deployment headroom</p><span data-view-state="readonly" className="mt-2 inline-flex rounded-full border border-teal-500/25 bg-teal-500/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-teal-300">Read-only</span></div>
            </div>
            <div className="mt-8 flex flex-wrap items-end gap-5">
              <div className="relative grid h-36 w-36 place-items-center rounded-full" style={{ background: `conic-gradient(${usageColors.solid} ${heroProgress}%, #17304e ${heroProgress}% 100%)` }}><div className="grid h-[108px] w-[108px] place-items-center rounded-full border border-slate-700/70 bg-[#071629] text-center"><div><p className="font-mono text-3xl font-bold text-slate-50">{safePercent(usage)}</p><p className="mt-1 text-[10px] uppercase tracking-[0.14em] text-slate-400">U Utilization</p></div></div></div>
              <div className="min-w-[240px] flex-1"><div className="flex flex-wrap items-center gap-3"><h3 className="font-display text-xl font-bold text-slate-100">Overall U Utilization</h3>{usageLevel && <span className={`rounded-full border px-3 py-1 text-xs font-bold ${usageColors.border} ${usageColors.bg} ${usageColors.text}`}>{usageLevel}</span>}</div><p className="mt-2 text-base text-slate-300"><span className="font-mono font-semibold text-slate-50">{formatFixedNumber(unitCapacityRow ? used : null, 0)} U</span> used of <span className="font-mono font-semibold text-slate-50">{formatFixedNumber(unitCapacityRow ? total : null, 0)} U</span></p><div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-800/90"><div className="h-full rounded-full shadow-[0_0_18px_currentColor]" style={{ width: `${heroProgress}%`, backgroundColor: usageColors.solid, color: usageColors.solid }} /></div><p className="mt-2 text-[11px] text-slate-500">{UTILIZATION_TOOLTIP}</p></div>
            </div>
          </div>
          <div className="hidden max-w-[250px] rounded-2xl border border-blue-400/15 bg-blue-950/45 p-5 backdrop-blur-sm xl:block"><BarChart3 className="h-7 w-7 text-blue-400" /><p className="mt-3 text-sm font-semibold text-slate-200">Capacity at a glance</p><p className="mt-2 text-xs leading-relaxed text-slate-400">{formatFixedNumber(unitCapacityRow ? available : null, 0)} U remain available for physical rack-space allocation.</p><p className="mt-3 text-[11px] text-slate-500">Data as of {dataAsOf ? safeDate(dataAsOf) : monthLabelLong(month, "en")}</p></div>
        </div>
      </header>

      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <MetricCard icon={Boxes} label="Total U" value={formatFixedNumber(unitCapacityRow ? total : null, 0)} sub="Installed rack-space capacity" accent="#60a5fa" sparkline={historySorted.map(row => row.totalU)} />
        <MetricCard icon={Activity} label="Used U" value={formatFixedNumber(unitCapacityRow ? used : null, 0)} sub={ofTotal(used)} accent="#6366f1" sparkline={historySorted.map(row => row.usedU)} />
        <MetricCard icon={CheckCircle2} label="Available U" value={formatFixedNumber(unitCapacityRow ? available : null, 0)} sub={ofTotal(Math.max(0, available))} accent="#14b8a6" sparkline={historySorted.map(row => row.availableU)} />
        <MetricCard icon={Gauge} label="Usage" value={safePercent(usage)} sub={usageLevel ? `${usageLevel} utilization` : "No current data"} accent={usageColors.solid} />
        <MetricCard icon={ShieldCheck} label="Availability" value={safePercent(availability)} sub="Physical rack-space headroom" accent="#2dd4bf" />
      </div>

      <div className="mt-4 grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="rounded-2xl border border-slate-800/80 bg-slate-900/80 p-5 shadow-[0_18px_50px_rgba(2,6,23,0.2)]"><div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-blue-400" /><h3 className="font-display text-base font-bold text-slate-100">Overall U Capacity Mix</h3></div><div className="mt-5"><UCapacityMix total={total} used={used} available={available} hasData={Boolean(unitCapacityRow)} /></div></section>
        <section className="rounded-2xl border border-slate-800/80 bg-slate-900/80 p-5 shadow-[0_18px_50px_rgba(2,6,23,0.2)]"><div className="flex items-center gap-2"><Gauge className="h-5 w-5 text-indigo-400" /><h3 className="font-display text-base font-bold text-slate-100">Operational Health</h3></div><div className="mt-4 grid gap-3 sm:grid-cols-2">
          <OperationalHealthCard icon={ShieldCheck} value={formatFixedNumber(unitCapacityRow ? available : null, 0)} label="U available for physical deployment" accent="#2dd4bf" tone="teal" />
          <OperationalHealthCard icon={Gauge} value={safePercent(usage)} label={usageLevel ? `${usageLevel} overall U utilization` : "No current utilization data"} accent={usageColors.solid} tone={usageLevel === "High" ? "rose" : usageLevel === "Attention" ? "amber" : "teal"} />
          <OperationalHealthCard icon={TrendingUp} value={trendPoints === null ? "—" : `${trendPoints >= 0 ? "+" : "−"}${formatFixedNumber(Math.abs(trendPoints), 1)} pp`} label="Usage change vs previous month" accent={trendPoints !== null && trendPoints > 0 ? "#fb7185" : "#34d399"} tone={trendPoints !== null && trendPoints > 0 ? "rose" : "teal"} />
          <OperationalHealthCard icon={Boxes} value={formatFixedNumber(unitCapacityRow ? total : null, 0)} label="Total installed rack units" accent="#60a5fa" tone="blue" />
        </div></section>
      </div>

      <section className="mt-4 overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-900/80 shadow-[0_18px_50px_rgba(2,6,23,0.2)]">
        <div className="flex flex-col gap-3 border-b border-slate-800 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><TrendingUp className="h-5 w-5 text-indigo-400" /><h3 className="font-display text-base font-bold text-slate-100">Rack Unit Capacity Trend</h3></div><p className="mt-1 text-[11px] text-slate-500">Total, Used and Available U across the selected trend period</p></div><div data-testid="rack-unit-trend-range" className="grid grid-cols-4 gap-1 rounded-xl border border-slate-800 bg-slate-950/60 p-1">{TREND_RANGE_OPTIONS.map(option => { const label = option === "All" ? "All" : option.replace("Last ", "").replace(" Months", "M"); return <button key={option} type="button" onClick={() => setSelectedTrend(option)} className={`min-h-9 rounded-lg px-3 text-[10px] font-bold transition-colors ${activeTrendRange === option ? "bg-indigo-600 text-white shadow-[0_0_18px_rgba(79,70,229,0.35)]" : "text-slate-400 hover:bg-slate-800 hover:text-slate-100"}`}>{label}</button>; })}</div></div>
        <div className="p-4 sm:p-5"><TrendLineChart labels={trendData.map(row => row.month)} unit="U" height={300} compact minPointSlots={12} series={[{ name: "Total U", color: "#60a5fa", values: trendData.map(row => row.total) }, { name: "Used U", color: "#6366f1", values: trendData.map(row => row.used) }, { name: "Available U", color: "#14b8a6", values: trendData.map(row => row.available) }]} /><p data-testid="rack-unit-capacity-trend-note" className="mt-4 rounded-xl border border-slate-800 bg-slate-950/35 px-4 py-3 text-xs leading-relaxed text-slate-400">{lang === "th" ? "หมายเหตุแนวโน้มความจุ Rack Unit: Available U แสดงเฉพาะพื้นที่ว่างทางกายภาพภายใน Rack เท่านั้น ความสามารถในการติดตั้งอุปกรณ์จริงยังขึ้นอยู่กับกำลังไฟฟ้า ระบบทำความเย็น น้ำหนัก และความต่อเนื่องของพื้นที่ว่างภายใน Rack" : "Rack Unit Capacity Trend Note: Available U represents physical rack space only; actual deployment capacity depends on power, cooling, weight, and contiguous space availability."}</p></div>
      </section>

      <section className="mt-4 overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-900/80 shadow-[0_18px_50px_rgba(2,6,23,0.2)]">
        <div className="border-b border-slate-800 px-5 py-4"><div className="flex items-center gap-2"><Boxes className="h-5 w-5 text-blue-400" /><h3 className="font-display text-base font-bold text-slate-100">Rack Unit Details</h3></div><p className="mt-1 text-[11px] text-slate-500">Monthly physical rack-space capacity history for the selected trend period</p></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[720px] border-collapse text-xs"><thead className="bg-slate-950/35 text-left text-[10px] uppercase tracking-wider text-slate-500"><tr>{["Month", "Total U", "Used U", "Available U", "Usage %", "Availability %"].map(label => <th key={label} className="border-b border-slate-800 px-4 py-3 font-semibold">{label}</th>)}</tr></thead><tbody>{detailRows.map(row => { const rowUsage = row.totalU > 0 ? (row.usedU / row.totalU) * 100 : null; const rowAvailability = row.totalU > 0 ? (row.availableU / row.totalU) * 100 : null; const rowLevel = rackUtilizationLevel(rowUsage); const rowColors = levelColors(rowLevel); return <tr key={row.month} className={`border-b border-slate-800/70 last:border-0 ${rowLevel === "High" ? "bg-rose-500/[0.035]" : ""}`}><td className="px-4 py-3 font-medium text-slate-300">{monthLabelLong(row.month, "en")}</td><td className="px-4 py-3 font-mono font-semibold text-blue-200">{formatFixedNumber(row.totalU, 0)}</td><td className="px-4 py-3 font-mono font-semibold text-indigo-200">{formatFixedNumber(row.usedU, 0)}</td><td className="px-4 py-3 font-mono font-semibold text-teal-300">{formatFixedNumber(row.availableU, 0)}</td><td className="px-4 py-3"><div className="flex items-center gap-2"><span className="font-mono text-slate-100">{safePercent(rowUsage)}</span><span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${rowColors.border} ${rowColors.text} ${rowColors.bg}`}>{rowLevel}</span></div><div className="mt-1.5 h-1.5 w-32 overflow-hidden rounded-full bg-slate-800"><div className="h-full rounded-full" style={{ width: `${clampPercent(rowUsage)}%`, backgroundColor: rowColors.solid }} /></div></td><td className="px-4 py-3 font-mono text-slate-200">{safePercent(rowAvailability)}</td></tr>; })}</tbody></table>{detailRows.length === 0 && <p className="py-8 text-center text-sm text-slate-500">No Rack Unit Capacity records are available for this range.</p>}</div>
      </section>

      <div className="mt-4 [&>section]:border-slate-800/80 [&>section]:bg-slate-900/80 [&>section]:shadow-[0_18px_50px_rgba(2,6,23,0.2)]"><RackUnitImage provider={imageProvider} facilityName={facilityName} month={month} /></div>
      <footer className="mt-4 flex flex-col gap-2 rounded-xl border border-slate-800/80 bg-slate-950/45 px-4 py-3 text-[11px] text-slate-500 sm:flex-row sm:items-center sm:justify-between"><span>U (Rack Unit) = Standard unit of vertical space in a rack. 1U = 1.75 inches.</span><span>Source: Production API · Snapshot: {monthLabelLong(month, "en")} · Last updated: {dataAsOf ? safeDate(dataAsOf) : "—"}</span></footer>
    </div>
  </div>;
}
