...
function MetricCard({ icon: Icon, label, value, accent, sparkline }: { icon: LucideIcon; label: string; value: string; accent: string; sparkline?: Array<number | null> }) {
  return <article className="min-h-[132px] rounded-xl border border-slate-800 bg-[#111c31] p-4 shadow-sm" style={{ borderTopColor: accent }}><div className="flex items-start justify-between gap-2"><div className="rounded-lg p-2" style={{ backgroundColor: `${accent}20`, color: accent }}><Icon className="h-4 w-4" /></div>{sparkline && <MiniSparkline values={sparkline} color={accent} />}</div><p className="mt-4 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">{label}</p><p className="mt-1 font-mono text-2xl font-semibold text-slate-100">{value}</p></article>;
}

function Section({ title, children, className = "" }: { title: string; children: ReactNode; className?: string }) {
  return <section className={`rounded-xl border border-slate-800 bg-[#0f1a2e] p-5 shadow-sm ${className}`}><div className="mb-4 flex items-center justify-between gap-3"><h3 className="font-display text-sm font-bold uppercase tracking-[0.08em] text-slate-200">{title}</h3></div>{children}</section>;
}
...
function RackCapacityDashboardInner({ siteId, month, snapshot, history, onSaved, onDirtyChange }: { siteId: number; month: string; snapshot: RackApiSnapshot | null; history: RackCapacityHistoryRow[]; onSaved?: (snapshot: RackApiSnapshot, history: RackCapacityHistoryRow[]) => void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  return <div className="space-y-5"><header className="rounded-xl border border-slate-800 bg-[#101b30] p-5"><div className="flex items-start gap-3"><div className="rounded-xl bg-rose-500/10 p-2.5 text-rose-300"><Server className="h-5 w-5" /></div><div><h2 className="font-display text-2xl font-bold tracking-tight text-slate-100">Rack Capacity &amp; Utilization — {monthLabelLong(month, "en")}</h2><p className="mt-1 text-sm text-slate-400">Rack capacity, status and utilization summary</p></div></div></header><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><MetricCard icon={Server} label="Total Racks" value={formatFixedNumber(metrics.total, 0)} accent="#64748b" /><MetricCard icon={CheckCircle2} label="In Use" value={formatFixedNumber(metrics.inUse.count, 0)} accent={STATUS_COLORS.inUse} /><MetricCard icon={ShieldCheck} label="Available" value={formatFixedNumber(metrics.available.count, 0)} accent={STATUS_COLORS.available} sparkline={totalHistory.map(row => row.available)} /><MetricCard icon={Clock3} label="Reserved" value={formatFixedNumber(metrics.reserved.count, 0)} accent={STATUS_COLORS.reserved} sparkline={totalHistory.map(row => row.reserved)} /><MetricCard icon={AlertTriangle} label="Pending Dismantle" value={formatFixedNumber(metrics.pendingDismantle.count, 0)} accent={STATUS_COLORS.pending} sparkline={totalHistory.map(row => row.pendingDismantle)} /></div><div className="grid gap-5 xl:grid-cols-[1.12fr_0.88fr]"><Section title="Overall Capacity Mix"><CapacityMix metrics={metrics} /></Section><Section title="Key Insights"><InsightRow icon={Gauge} accent="#60a5fa">Utilization is {safePercent(ratioPercent(metrics.inUse.ratio))} ({metrics.inUse.count} / {metrics.total} racks).</InsightRow><InsightRow icon={TrendingUp} accent="#f472b6">{zoneGrammar(qualifyingZones)}</InsightRow><InsightRow icon={Clock3} accent="#f59e0b">{highestPending && highestPending.pendingDismantle.count > 0 ? `${highestPending.zone} has the highest pending dismantle count (${highestPending.pendingDismantle.count} racks).` : "No racks are pending dismantle."}</InsightRow><InsightRow icon={AlertTriangle} accent="#f59e0b">{metrics.available.count} racks are available for allocation.</InsightRow></Section></div><ZoneBreakdown metrics={metrics} /><p className="text-center text-[11px] text-slate-500">Last updated: {safeDate(updatedAt)}</p><WebRackCapacityEditor siteId={siteId} month={month} snapshot={snapshot} onSaved={onSaved} onDirtyChange={onDirtyChange} /></div>;
}
