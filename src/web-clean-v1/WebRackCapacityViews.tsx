...
export function WebRackCapacityDashboard({ siteId, siteName, month, snapshot, rackCapacityHistory, rackUnitCapacity, onGoToEntry }: { siteId: number; siteName: string | null; month: string; snapshot: RackApiSnapshot | null; rackCapacityHistory: RackCapacityHistoryRow[]; rackUnitCapacity: RackUnitCapacityRow[]; onGoToEntry?: () => void }) {
  if (!snapshot) return <section role="status" className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-5 text-sm text-amber-100"><p>No confirmed Rack Capacity snapshot for {monthLabelLong(month, "en")}.</p><p className="mt-1">Review or confirm this month in Monthly Data Entry.</p>{onGoToEntry && <button type="button" onClick={onGoToEntry} className="mt-4 rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white hover:bg-indigo-500">Go to Monthly Data Entry</button>}</section>;
...
function UCapacityMix({ total, used, available, hasData }: { total: number; used: number; available: number; hasData?: boolean }) {
  ...
  return <div className="space-y-4"><div className="flex h-8 overflow-hidden rounded-lg bg-slate-800"><div className="h-full bg-orange-400" style={{ width: `${usedWidth}%` }} /><div className="h-full bg-emerald-400" style={{ width: `${availableWidth}%` }} /></div><div className="grid gap-3 sm:grid-cols-2"><div className="flex items-center justify-between text-xs"><span className="inline-flex items-center gap-2 text-slate-300"><i className="h-2.5 w-2.5 rounded-full bg-orange-400" />Used ({formatFixedNumber(used, 1)} U)</span><span className="font-mono text-slate-100">{safePercent(usagePct)}</span></div><div className="flex items-center justify-between text-xs"><span className="inline-flex items-center gap-2 text-slate-300"><i className="h-2.5 w-2.5 rounded-full bg-emerald-400" />Available ({formatFixedNumber(Math.max(0, available), 1)} U)</span><span className="font-mono text-slate-100">{safePercent(usagePct === null ? null : 100 - usagePct)}</span></div></div><div className="border-t border-slate-800 pt-3 text-xs text-slate-400">You are using <span className="font-mono text-slate-200">{safePercent(usagePct)}</span> of total capacity. Available capacity is <span className="font-mono text-slate-200">{formatFixedNumber(Math.max(0, available), 1)} U</span>.</div></div>;
}
...
function RackUnitCapacityDashboardInner({ month, facilityName, imageProvider, history }: { month: string; facilityName: string | null; imageProvider?: Pick<IDataProvider, "getRackUnitCapacityImage"> | null; history: RackCapacityHistoryRow[] }) {
