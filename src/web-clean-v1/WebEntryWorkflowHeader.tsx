...
export function WebHistoricalEditNotice({ selectedMonth, latestMonth, onReturnToLatest }: {
  selectedMonth: string;
  latestMonth: string | null;
  onReturnToLatest: () => void;
}) {
  if (!latestMonth || selectedMonth === latestMonth) return null;
  return <section role="status" className="mt-4 flex flex-col items-start justify-between gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-xs text-amber-200 sm:flex-row sm:items-center"><div className="flex items-start gap-2.5"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-amber-400" /><div><b className="block text-[10px] uppercase tracking-wider text-amber-400">Historical Data Edit Mode</b><span>You are editing {selectedMonth}, not the latest available record ({latestMonth}). Review changes before saving.</span></div></div><button type="button" onClick={onReturnToLatest} className="shrink-0 rounded-lg bg-amber-500/20 px-3 py-1.5 font-semibold text-amber-100 hover:bg-amber-500/30">Return to latest</button></section>;
}
