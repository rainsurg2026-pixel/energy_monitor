...
export default function RackUnitCapacityEntry({ siteId, month, initialRow, onSaved, onMessage, onCompletionChange }: Props) {
  ...
  const save = async () => {
    ...
    if (!Number.isFinite(total) || total < 0 || !Number.isFinite(used) || used < 0) { onMessage("Enter valid non-negative Total (U) and Used (U) values."); return; }
    setBusy(true);
    try {
  ...
  return <section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-sm">
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      <label className="text-xs text-slate-400">Total (U)<input type="number" min="0" value={totalU} onChange={event => setTotalU(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-200" /></label>
      <label className="text-xs text-slate-400">Used (U)<input type="number" min="0" value={usedU} onChange={event => setUsedU(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-200" /></label>
