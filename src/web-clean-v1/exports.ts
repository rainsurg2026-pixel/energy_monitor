...
async function workbookForCurrentFacility(facility: ExportFacility): Promise<any> {
  ...
  const savedEnergy = addCurrentTableSheet(workbook, "02_Saved_Energy", "tblSavedEnergy",
    ...
    metrics.map(metric => [...currentMetricRows([metric])[0], metric.floorEnergyKwh === null ? "Partial" : "Complete"]), "saved");
  ...
  addCurrentTableSheet(workbook, "05_Input_UPS", "tblInputUPS", ["Month", "Facility", "UPS ID", "Voltage (V)", "Current (A)", "Load (kW)", "Load (kVA)", "Raw phases JSON", "Last Saved"], logs.flatMap(log => log.ups.map(row => [log.month, facility.siteName, row.upsId, row.voltage, row.current, row.loadKw, row.loadKva, JSON.stringify(row.phases ?? {}), log.lastSavedUps])), "input");
  ...
  addCurrentTableSheet(workbook, "07_Input_DCPower", "tblInputDCPower", ["Month", "Facility", "DC Panel", "Voltage (V)", "Current (A)", "Last Saved"], logs.flatMap(log => log.dc.map(row => [log.month, facility.siteName, row.panelId, row.voltage, row.current, log.lastSavedDc])), "input");
  const inputEnergyCost = addCurrentTableSheet(workbook, "08_Input_EnergyCost", "tblInputEnergyCost", ["Month", "Facility", "Building Energy (kWh)", "Building Cost (THB)", "Stored 4th Floor Cost (THB)", "Stored Average Rate (THB/kWh)", "Last Saved"], logs.map(log => [log.month, facility.siteName, log.energyCost.buildingEnergyKwh, log.energyCost.buildingElectricityCostThb, log.energyCost.floorElectricityCostThb ?? null, log.energyCost.averageElectricityRateThbPerKwh ?? null, log.lastSavedEnergyCost]), "input");
  ...
  addCompatibilityTableSheet(workbook, "", "UPS_Loads", ["Month", "UPS ID", "Voltage (V)", "Current (A)", "Load (kW)", "Load (kVA)", "Raw phases JSON", "Last Saved"], logs.flatMap(log => log.ups.map(row => [log.month, row.upsId, row.voltage, row.current, row.loadKw, row.loadKva, JSON.stringify(row.phases ?? {}), log.lastSavedUps])));
  const legacyAir = addCompatibilityTableSheet(workbook, "", "Air_Inputs", ["Month", ...airFields.map(field => field.toUpperCase() + " (GWh)"), "Raw meters JSON", "Last Saved"], airRows.map(row => [row.month, ...row.values, JSON.stringify(logs.find(log => log.month === row.month)?.air.meters ?? {}), logs.find(log => log.month === row.month)?.lastSavedAir ?? null]));
  ...
  addCompatibilityTableSheet(workbook, "", "DC_Inputs", ["Month", "DC Panel", "Voltage (V)", "Current (A)", "Last Saved"], logs.flatMap(log => log.dc.map(row => [log.month, row.panelId, row.voltage, row.current, log.lastSavedDc])));
  ...
  addCompatibilityTableSheet(workbook, "", "Saved_Records", ["Month", "UPS Last Saved", "Air Last Saved", "DC Last Saved", "Energy Cost Last Saved", "Source State"], months.map(month => {
  ...
  addCompatibilityTableSheet(workbook, "", "Saved_Values", ["Month", "UPS Saved JSON", "Air Saved JSON", "DC Saved JSON", "Energy Cost Saved JSON", "Rack Unit Saved JSON", "Rack Unit Image JSON", "UPS Last Saved", "Air Last Saved", "DC Last Saved", "Energy Cost Last Saved"], months.map(month => {
    const log = logsByMonth.get(month);
  ...
  addCompatibilityTableSheet(workbook, "", "Raw_Inputs", ["Month", "Raw Phase/Panel Values JSON"], logs.filter(log => log.srinakarinInputs).map(log => [log.month, JSON.stringify(log.srinakarinInputs)]));
  ...
  const legacyRackUnit = addCompatibilityTableSheet(workbook, "", "Rack Unit Capacity", ["Month", "Total (U)", "Used (U)", "Available (U)", "Usage (%)", "Availability (%)", "Image Attached", "Image Content Type", "Image Saved At"], (facility.rackUnitCapacity ?? []).filter(row => months.includes(row.month)).map(row => [row.month, row.totalU, row.usedU, row.availableU, row.totalU > 0 ? row.usedU / row.totalU : null, row.availabilityPct, row.imageAttached ? "Yes" : "No", row.imageContentType ?? null, row.imageSavedAt ?? null]));
  ...
  if ((facility.rackUnitCapacityImages ?? []).length > 0) {
    const imageMetadata = addPresentationSheet(workbook, "15_History_RackImage", "Rack Unit Capacity image metadata");
