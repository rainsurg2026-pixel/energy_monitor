...
export function buildAllFacilitiesReportHtml(facilities: ExportFacility[], comparison: SiteComparisonReportModel | null, selectedMonth: string, sections?: readonly ReportSectionId[]): string {
  ...
  const audit = `${auditSource?.generatedBy ? `<br>Generated by: ${escapeHtmlLocal(auditSource.generatedBy)}` : ""}${auditSource?.generatedAt ? `<br>Generated at: ${escapeHtmlLocal(auditSource.generatedAt)}` : ""}`;
  const cover = `<main class="cover"><h1>Data Center Energy &amp; Facility Monitor</h1><h2>All Facilities Report</h2><div class="meta">Facility: All Facilities<br>Reporting month: ${escapeHtmlLocal(selectedMonth)}<br>Sites: ${sites}${audit}</div></main>`;
