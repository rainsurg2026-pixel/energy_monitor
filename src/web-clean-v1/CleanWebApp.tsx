...
import { api, setUnauthorizedHandler, type SessionUser } from "./api";
...
import { defaultAllFacilitiesReportFilename, defaultReportFilename, resolveFilename, withExtension } from "./reportFilename";
...
import ApplicationSettings, { defaultFacilityStorageKey } from "./ApplicationSettings";
import AppNavigationV2, { type AppViewV2 } from "./AppNavigationV2";
...
const readStoredFacility = (userId: string) => { try { return sessionStorage.getItem(facilityStorageKey(userId)) ?? localStorage.getItem(defaultFacilityStorageKey(userId)); } catch { return null; } };
...
export default function CleanWebApp() {
  ...
  return <ReportProvider syncedLogs={history.logs} availableMonths={reportAvailableMonths} onYearChange={loadHistoricalYear} displayPeriod={bootstrap?.displayPeriod.startMonth.slice(0, 4)}>
    <div className="em-shell min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="mx-auto max-w-[1600px] px-3 py-3 sm:px-4 sm:py-4 md:px-6 md:py-6"><AppNavigationV2 activeView={view} lang={lang} isAdmin={user.role === "admin"} onNavigate={setView} onLogout={logout} />
        <main className="min-w-0 pb-20 md:pb-6">{view !== "dashboard" && <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-900/60 p-3"><div><span className="text-xs uppercase tracking-wide text-slate-500">{shellCopy.reportingMonth}</span><div className="text-lg font-semibold">{month}</div></div><input aria-label={shellCopy.reportingMonth} type="month" value={month} min={bootstrap?.displayPeriod.startMonth} max={bootstrap ? (bootstrap.displayPeriod.endMonth < todayMonth() ? bootstrap.displayPeriod.endMonth : todayMonth()) : todayMonth()} onChange={event => void selectMonth(event.target.value, history.months.includes(event.target.value))} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" /><div className="text-right text-xs text-slate-400">{shellCopy.displayPeriod} {bootstrap?.displayPeriod.startMonth} to {bootstrap?.displayPeriod.endMonth}<br />{shellCopy.completion} <b className="text-teal-300">{completion.overall.percent}%</b></div></div>}
          ...
          {view === "settings" ? <ApplicationSettings lang={lang} displayPeriod={settingsDisplayPeriod} isAdmin={user.role === "admin"} theme={theme} readOnlyMode={bootstrap?.readOnlyMode ?? false} userId={user.id} userDisplayName={user.displayName} sites={(bootstrap?.sites ?? []).map(item => ({ id: item.id, code: item.code, name: item.name }))} onThemeChange={changeTheme} onLanguageChange={changeLanguage} onSaved={async () => { await refreshAfterSettings(); }} onMessage={setNotice} /> : view === "admin" && user.role === "admin" ? <AdminUserManagement lang={lang} /> : facilityError ? <section role="alert" className="rounded-xl border border-rose-500/40 bg-rose-500/10 p-5 text-rose-100"><h2 className="font-semibold">{lang === "th" ? "ไม่สามารถโหลดบริบทไซต์ได้" : "Facility context unavailable"}</h2><p className="mt-2 text-sm">{facilityError}</p><button onClick={() => void initialize().catch(() => undefined)} className="mt-4 rounded-lg border border-rose-300/50 px-3 py-2 text-sm">{lang === "th" ? "ลองโหลดใหม่" : "Retry facility load"}</button></section> : facilityLoading || !site ? <section className="rounded-xl border border-slate-800 bg-slate-900 p-5 text-sm text-slate-300">{lang === "th" ? "กำลังโหลดข้อมูลไซต์…" : "Loading facility context…"}</section> : <>{view === "dashboard" && <DashboardView logs={history.logs} month={displayMonth} displayPeriod={globalDisplayPeriodRange} siteName={site.name} siteCode={site.code} lang={lang} upsGroupHistory={historyUpsGroupHistory} rackCapacityHistory={historyRackCapacityHistory} rackUnitCapacity={historyRackUnitCapacity} availableMonths={siteMonthsAvailable} onSelectMonth={selected => void selectMonth(selected, history.months.includes(selected))} onViewRackCapacity={() => setView("racks")} onViewRackUnitCapacity={() => setView("rack-units")} onRefresh={refreshReports} onNotice={setNotice} />}
          {view === "entry" && draft && <WebEntryWorkspace lang={lang} siteId={siteId!} siteName={site.name} siteCode={site.code} months={history.months} month={month} draft={draft} historyLogs={history.logs} rackUnitInitialRow={history.rackUnitCapacity?.find(row => row.month === month) ?? null} busy={busy} readOnly={bootstrap?.readOnlyMode ?? false} allowedStartMonth={bootstrap?.displayPeriod.startMonth ?? month} allowedEndMonth={bootstrap ? (bootstrap.displayPeriod.endMonth < todayMonth() ? bootstrap.displayPeriod.endMonth : todayMonth()) : month} onSave={save} onSelectMonth={(selected, exists) => void selectMonth(selected, exists)} onRackUnitSaved={async () => { if (!siteId) return; const records = await loadHistory(siteId, { force: true, scope: "dashboard" }); await loadMonth(siteId, month, records); }} onRackCapacitySaved={async () => { if (!siteId) return; historyCacheRef.current.delete(String(siteId) + ":full"); await loadHistory(siteId, { force: true, scope: "rack", prefetch: true }); }} onNotice={setNotice} onDirtyChange={setEntryDirty} onRegisterActions={registerEntryActions} />}
          ...
          {view === "reports" && <Reports lang={lang} siteId={siteId} siteName={site?.name ?? "Data Center Energy & Facility Monitor"} userDisplayName={user.displayName} logs={history.logs} month={displayMonth} sites={bootstrap?.sites ?? []} monthsAvailable={siteMonthsAvailable} rackCapacityHistory={historyRackCapacityHistory} rackUnitCapacity={historyRackUnitCapacity} upsGroupHistory={historyUpsGroupHistory} onRefresh={refreshReports} />}
          </>}
           </Suspense>
         </main></div>
...
function DashboardView({ logs, month, displayPeriod, siteName = "Facility", siteCode = "", dashboardMapping, lang, upsGroupHistory, rackCapacityHistory, rackUnitCapacity, availableMonths, onSelectMonth, onViewRackCapacity, onViewRackUnitCapacity, onRefresh, onNotice }: { logs: MonthlyLog[]; month: string; displayPeriod?: string; siteName?: string; siteCode?: string; dashboardMapping?: DashboardUpsMappingReport | null; lang: "th" | "en"; upsGroupHistory: UpsGroupHistoryReport | null; rackCapacityHistory: RackCapacityHistoryRow[]; rackUnitCapacity: RackUnitCapacityRow[]; availableMonths: readonly string[]; onSelectMonth?: (month: string) => void; onViewRackCapacity?: () => void; onViewRackUnitCapacity?: () => void; onRefresh?: () => void | Promise<void>; onNotice?: (message: string) => void }) {
...
function Reports({ lang, siteId, siteName, userDisplayName, logs, month, sites, monthsAvailable, rackCapacityHistory, rackUnitCapacity, upsGroupHistory, onRefresh }: { lang: AppLanguage; siteId: number | null; siteName: string; userDisplayName: string; logs: MonthlyLog[]; month: string; sites: Site[]; monthsAvailable: readonly string[]; rackCapacityHistory: RackCapacityHistoryRow[]; rackUnitCapacity: RackUnitCapacityRow[]; upsGroupHistory: UpsGroupHistoryReport | null; onRefresh: () => Promise<void> }) {
  const th = lang === "th";
  const exportAudit = () => ({ generatedBy: userDisplayName, generatedAt: new Date().toISOString() });
  ...
  const loadComparison = useCallback(async () => {
    ...
    const request = api<SiteComparisonExport>("/site-comparison").then(async result => {
      const chartMonths = selectedReportMonths.length === 1
        ? recentMonthsThroughSelected(result.months, selectedMonth, 12)
        : selectedReportMonths;
      const chartMonthSet = new Set(chartMonths);
      const sitesWithRack = await Promise.all(result.sites.map(async item => {
        ...
        return {
          ...
          months: item.months.filter(entry => chartMonthSet.has(entry.month)),
      ...
      return {
        ...result,
        months: result.months.filter(value => chartMonthSet.has(value)),
        sites: sitesWithRack
      };
    });
    ...
    return request;
  }, [loadRack, period, periodEndMonth, periodIdentity, selectedReportMonths]);
  ...
  return <section>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-2">{cards("current", reportCopy.current, `${siteName}, ${reportingPeriodLabel(period, lang)}.`, { csv: () => (async () => { const rack = siteId !== null ? rackReportFromSnapshot(await loadRack(siteId, contextMonth)) : null; exportCsv(scopedLogs, siteName, withExtension(resolvedFileName, "csv"), { calculationLogs: logs, rack, rackHistory: scopedRackCapacityHistory, rackUnitCapacity: scopedRackUnitCapacity, upsGroupHistory: scopedUpsGroupHistory }); })(), excel: () => (async () => { const rack = siteId !== null ? rackReportFromSnapshot(await loadRack(siteId, contextMonth)) : null; const fullHistory = siteId === null ? null : await api<HistoryData>("/sites/" + siteId + "/history?scope=full").catch(() => null); const image = await resolveReportImageForExport(); const audit = exportAudit(); const calculationLogs = fullHistory?.logs ?? logs; const exportLogs = filterLogsByPeriod(calculationLogs, period, periodEndMonth); const fullRackHistory = fullHistory?.rackCapacityHistory ?? rackCapacityHistory; const fullRackUnitCapacity = fullHistory?.rackUnitCapacity ?? rackUnitCapacity; const fullUpsGroupHistory = fullHistory?.upsGroupHistory ?? upsGroupHistory; await exportExcel(exportLogs, siteName, withExtension(resolvedFileName, "xlsx"), { calculationLogs, rack, rackHistory: fullRackHistory, rackUnitCapacity: fullRackUnitCapacity, trendRackUnitCapacity: fullRackUnitCapacity, upsGroupHistory: fullUpsGroupHistory, reportingMonths: selectedReportMonths, selectedMonth: contextMonth, rackUnitCapacityImageDataUri: image?.dataUri ?? null, rackUnitCapacityImageMeta: image?.meta ?? null, ...audit }); })(), html: () => (async () => { const rack = siteId !== null ? rackReportFromSnapshot(await loadRack(siteId, contextMonth)) : null; await exportHtml(scopedLogs, siteName, contextMonth, withExtension(resolvedFileName, "html"), rack, scopedRackCapacityHistory, scopedRackUnitCapacity, logs, selectedReportSections, { upsGroupHistory: scopedUpsGroupHistory }); })(), pdf: () => (async () => { const rack = siteId !== null ? rackReportFromSnapshot(await loadRack(siteId, contextMonth)) : null; const audit = exportAudit(); await exportDesktopPdf(scopedLogs, siteName, contextMonth, resolvedFileName, rack, scopedRackCapacityHistory, scopedRackUnitCapacity, logs, selectedReportSections, { upsGroupHistory: scopedUpsGroupHistory, ...audit }); })() }, { csv: withExtension(resolvedFileName, "csv"), excel: withExtension(resolvedFileName, "xlsx"), html: withExtension(resolvedFileName, "html"), pdf: withExtension(resolvedFileName, "pdf") })}{cards("all", reportCopy.all, reportCopy.allDesc, { csv: async () => exportAllFacilitiesCsv(await loadAll({ includeRack: true, includeImage: false }), buildSiteComparisonReportModel(await loadComparison(), contextMonth), withExtension(allFacilitiesFileName, "csv")), excel: async () => { const audit = exportAudit(); const facilities = (await loadAll({ includeRack: true, includeImage: true })).map(facility => ({ ...facility, ...audit })); return exportAllFacilitiesExcel(facilities, buildSiteComparisonReportModel(await loadComparison(), contextMonth), withExtension(allFacilitiesFileName, "xlsx")); }, html: async () => exportAllFacilitiesHtml(await loadAll({ includeRack: true, includeImage: true }), buildSiteComparisonReportModel(await loadComparison(), contextMonth), contextMonth, withExtension(allFacilitiesFileName, "html")), pdf: async () => { const audit = exportAudit(); const facilities = (await loadAll({ includeRack: true, includeImage: true })).map(facility => ({ ...facility, ...audit })); return exportAllFacilitiesPdf(facilities, buildSiteComparisonReportModel(await loadComparison(), contextMonth), contextMonth, withExtension(allFacilitiesFileName, "pdf")); } }, { csv: withExtension(allFacilitiesFileName, "csv"), excel: withExtension(allFacilitiesFileName, "xlsx"), html: withExtension(allFacilitiesFileName, "html"), pdf: withExtension(allFacilitiesFileName, "pdf") })}</div>
