...
import type { DashboardUpsMappingReport, EngineeringDashboardSnapshot } from "../reports/reportTypes";
...
function normalizedUpsId(value: string): string {
...
/** The one authoritative selected-month Engineering Dashboard calculation. */
export function buildEngineeringDashboardSnapshot(
  ...
  const upsGroups = topology?.upsGroups?.length
    ? computeUpsGroupSummary(activeLog, topology.upsGroups).map(row => ({
      name: row.name, totalKw: row.totalLoadKw, totalKva: row.totalLoadKva, capacity: row.capacity,
      loadPercent: row.loadPercent, availablePercent: row.availablePercent, monthlyEnergyKwh: row.monthlyEnergyKwh
    }))
  ...
  const upsOverallGroups = hasAcPowerPanel
    ? computeUpsGroupSummary(activeLog, SRINAKARIN_OVERALL_GROUPS).map(row => ({
      name: row.name, totalKw: row.totalLoadKw, totalKva: row.totalLoadKva, capacity: row.capacity,
      loadPercent: row.loadPercent, availablePercent: row.availablePercent, monthlyEnergyKwh: row.monthlyEnergyKwh
    }))
    : [];
