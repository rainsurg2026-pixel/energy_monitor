...
export function normalizedMonth(month: string): string | null {
  ...
  if (numericMatch) {
    ...
    const monthIndex = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"]
      .indexOf(shortMatch[1].toLowerCase());
...
function calculateUpsEnergy(log: MonthlyLog, days: number): number | null {
  ...
  const groupLoads = groups.map(group =>
    sumRequired(group.map(id => findUps(log, id)?.loadKw ?? null))
  );
...
/**
 * Dashboard-FAC calculation mapping:
 * - H2 = DAY(EOMONTH(H1,0)); UPS H4:H7 = group load * 24 * H2; H8 = SUM(H4:H7).
 * - F20 = SUM(B22:E22) * 1000000, where B22:E22 are current minus previous
 *   calendar-month meter readings from `2. Air Energy Consumption Log`.
 * - G25:G28 = (voltage * current / 200) * 220 / 1000 * 24 * H2;
 *   G29 = SUM(G25:G28).
 * - D32 = H8 + F20 + G29; F32 = C32 / B32; E32 = F32 * D32; G32 = D32 / B32.
 * Building energy (B32) and building cost (C32) are the raw values from
 * `4. Electricity Cost Log` / `Overall_Energy`; Dashboard-FAC cached values
