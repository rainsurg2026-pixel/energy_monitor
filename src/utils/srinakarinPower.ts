...
/**
 * The Srinakarin workbook stores phase-level inputs in several purple tabs,
 * while the monthly aggregate is written to `1. UPS Data Log`.  Keep the
 * aggregation here so the entry screen, workbook writer and reports share the
 * same month/device matching rules.
 */
...
function phaseValues(
  snapshot: SrinakarinInputSnapshot | undefined,
  source: "upsPhase" | "acPhase",
  prefix: string,
...
function ppc43Current(snapshot: SrinakarinInputSnapshot | undefined, prefix: string): number | null {
  // Dashboard-FAC averages the three phase totals. Each phase total is the
  // SUMIFS result for `PPC 43A - R`, `PPC 43A - S`, or `PPC 43A - T`.
...
/** Calculate the rows that belong in Srinakarin's `1. UPS Data Log`. */
